...
class StegaStampEncoder(nn.Module):
    def __init__(self):
        ...
        self.secret_dense = Dense(100, 3*8*8, activation='relu', kernel_initializer='he_normal')  # 输出3x8x8=192
        
        self.conv1 = Conv2D(6, 32, 3, activation='relu')           # 32x32
        self.conv2 = Conv2D(32, 64, 3, activation='relu', strides=2) # 16x16
        self.conv3 = Conv2D(64, 128, 3, activation='relu', strides=2) # 8x8
        
        self.up4 = Conv2D(128, 64, 3, activation='relu')
        self.conv4 = Conv2D(128, 64, 3, activation='relu')  # 合并后通道数
        
        self.up5 = Conv2D(64, 32, 3, activation='relu')
        self.conv5 = Conv2D(64, 32, 3, activation='relu')  # 合并后通道数
        
        self.residual = Conv2D(32, 3, 1, activation=None)

        # self.secret_dense = Dense(100, 48, activation='relu', kernel_initializer='he_normal')
    ...
    def forward(self, inputs):
        ...
        # secrect = secrect.reshape(-1, 3, 4, 4)
        secrect = secrect.reshape(-1, 3, 8, 8)
        secrect_enlarged = nn.Upsample(scale_factor=4)(secrect)

        inputs = torch.cat([secrect_enlarged, image], dim=1)
        conv1 = self.conv1(inputs)        # 32x32
        conv2 = self.conv2(conv1)         # 16x16
        conv3 = self.conv3(conv2)         # 8x8

        # 解码器路径
        up4 = self.up4(nn.Upsample(scale_factor=2)(conv3))  # 16x16
        merge4 = torch.cat([conv2, up4], dim=1)             # 64+64=128
        conv4 = self.conv4(merge4)                          # 16x16

        up5 = self.up5(nn.Upsample(scale_factor=2)(conv4))  # 32x32
        merge5 = torch.cat([conv1, up5], dim=1)             # 32+32=64
        conv5 = self.conv5(merge5)                          # 32x32

        residual = self.residual(conv5)
...
class SpatialTransformerNetwork(nn.Module):
    def __init__(self):
        ...
        self.localization = nn.Sequential(
            Conv2D(3, 32, 3, strides=2, activation='relu'),    # 32x32 -> 16x16
            Conv2D(32, 64, 3, strides=2, activation='relu'),   # 16x16 -> 8x8
            Conv2D(64, 128, 3, strides=2, activation='relu'),  # 8x8 -> 4x4
            Flatten(),
            # 修改全连接输入维度为128*4*4=2048
            Dense(128*4*4, 128, activation='relu'),
            nn.Linear(128, 6)
        )
...
class StegaStampDecoder(nn.Module):
    def __init__(self, secret_size=100):
        ...
        self.decoder = nn.Sequential(
            Conv2D(3, 32, 3, strides=2, activation='relu'),    # 32x32 -> 16x16
            # Conv2D(32, 32, 3, activation='relu'),              # 16x16
            Conv2D(32, 64, 3, strides=2, activation='relu'),   # 16x16 -> 8x8
            # Conv2D(64, 64, 3, activation='relu'),              # 8x8
            # Conv2D(64, 64, 3, strides=2, activation='relu'),   # 8x8 -> 4x4
            Conv2D(64, 128, 3, strides=2, activation='relu'),  # 4x4 -> 2x2
            # Conv2D(128, 128, 3, strides=2, activation='relu'), # 2x2 -> 1x1
            Conv2D(128, 256, 3, strides=2, activation='relu'), # 2x2 -> 1x1
            Flatten(),
            Dense(256*2*2, 512, activation='relu'),
