...
def transform_net(encoded_image, args, global_step):
    ...
    rnd_brightness = add_utils.get_rnd_brightness_torch(rnd_bri, rnd_hue, args.batch_size)  # [batch_size, 3, 1, 1]
    ...
    # blur
    N_blur = 7
    f = add_utils.random_blur_kernel(probs=[.25, .25], N_blur=N_blur, sigrange_gauss=[1., 3.], sigrange_line=[.25, 1.],
                                 wmin_line=3)
    if args.cuda:
        f = f.cuda()
    ...
    if not args.no_jpeg:
        encoded_image = add_utils.jpeg_compress_decompress(encoded_image, rounding=add_utils.round_only_at_0,
